// Package summary's Processor builds the per-week reporting projection from
// planned items + performances + the joined exercise/theme/region rows.
// Nothing is persisted: the endpoint is a pure read. The Processor lives in
// its own file so the projection logic can be tested without spinning up an
// HTTP server.
package summary
...
func ensureThemeGroup(m map[uuid.UUID]*themeGroup, themeMap map[uuid.UUID]theme.Entity, id uuid.UUID) *themeGroup {
	if g, ok := m[id]; ok {
		return g
	}
	name := ""
...
func ensureRegionGroup(m map[uuid.UUID]*regionGroup, regionMap map[uuid.UUID]region.Entity, id uuid.UUID) *regionGroup {
	if g, ok := m[id]; ok {
		return g
	}
	name := ""
