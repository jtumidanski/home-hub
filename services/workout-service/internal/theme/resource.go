...
func InitializeRoutes(db *gorm.DB) func(l logrus.FieldLogger, si jsonapi.ServerInformation, api *mux.Router) {
	return func(l logrus.FieldLogger, si jsonapi.ServerInformation, api *mux.Router) {
		...
		api.HandleFunc("/workouts/themes", rh("ListThemes", listHandler(db))).Methods(http.MethodGet)
		api.HandleFunc("/workouts/themes", rihCreate("CreateTheme", createHandler(db))).Methods(http.MethodPost)
		api.HandleFunc("/workouts/themes/{id}", rihUpdate("UpdateTheme", updateHandler(db))).Methods(http.MethodPatch)
		api.HandleFunc("/workouts/themes/{id}", rh("DeleteTheme", deleteHandler(db))).Methods(http.MethodDelete)
	}
}
...
func listHandler(db *gorm.DB) server.GetHandler {
	return func(d *server.HandlerDependency, c *server.HandlerContext) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			if err != nil {
				d.Logger().WithError(err).Error("Failed to list themes")
	...
	return func(d *server.HandlerDependency, c *server.HandlerContext, input CreateRequest) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			if err != nil {
				...
				d.Logger().WithError(err).Error("Failed to create theme")
				server.WriteError(w, http.StatusInternalServerError, "Error", "")
				return
			}
			...
			return func(w http.ResponseWriter, r *http.Request) {
				...
				if err != nil {
					...
					case errors.Is(err, ErrNotFound):
						server.WriteError(w, http.StatusNotFound, "Not Found", "Theme not found")
		...
		return server.ParseID("id", func(id uuid.UUID) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				if err := proc.Delete(id); err != nil {
					...
					d.Logger().WithError(err).Error("Failed to delete theme")
					server.WriteError(w, http.StatusInternalServerError, "Error", "")
					return
				}
