...
func TestCreate_NormalizesCoordinatesToFourDecimals(t *testing.T) {
	db := setupTestDB(t)
	w := &recordingWarmer{}
	...
	m, err := p.Create(tenantID, householdID, CreateInput{
		PlaceName: "Somewhere",
...
func TestCreate_TrimsAndStoresLabel(t *testing.T) {
	db := setupTestDB(t)
...
func TestCreate_RejectsLabelOver64Chars(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
...
func TestCreate_EmptyLabelStoredAsNil(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
...
func TestCreate_CapAtTenReturnsErrCapReached(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
...
func TestCreate_CapIsPerHousehold(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
	...
	// Household B should still be able to create locations.
...
func TestGet_MissingIdReturnsNotFound(t *testing.T) {
	...
	_, err := p.Get(uuid.New(), uuid.New())
	if err != ErrNotFound {
		t.Errorf("expected ErrNotFound, got %v", err)
	}
}

func TestGet_CrossHouseholdReturnsNotFound(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
	...
	_, err = p.Get(otherHousehold, m.Id())
	if err != ErrNotFound {
...
func TestList_OnlyReturnsHouseholdRows(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
...
func TestUpdateLabel_RejectsTooLong(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
	...
	_, err = p.UpdateLabel(householdID, m.Id(), &long)
	if err != ErrLabelTooLong {
		t.Errorf("expected ErrLabelTooLong, got %v", err)
	}
}

func TestUpdateLabel_EmptyClearsLabel(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
	...
	empty := ""
	updated, err := p.UpdateLabel(householdID, m.Id(), &empty)
	if err != nil {
...
func TestUpdateLabel_CrossHouseholdReturnsNotFound(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
	...
	m, err := p.Create(tenantID, owner, CreateInput{
		PlaceName: "X", Latitude: 0, Longitude: 0,
	})
	if err != nil {
		t.Fatalf("setup failed: %v", err)
	...
	label := "Sneaky"
	_, err = p.UpdateLabel(other, m.Id(), &label)
	if err != ErrNotFound {
		t.Errorf("expected ErrNotFound, got %v", err)
	}
}

func TestDelete_CrossHouseholdReturnsNotFound(t *testing.T) {
	db := setupTestDB(t)
	p := newTestProcessor(t, db, nil)
...
func TestCreate_WarmerErrorIsSwallowed(t *testing.T) {
	db := setupTestDB(t)
