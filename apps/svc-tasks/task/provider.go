...
// GetById returns a provider that fetches a task by ID
...
// GetByUserId returns a provider that fetches all tasks for a user
...
// GetByUserIdAndDay returns a provider that fetches all tasks for a user on a specific day
func GetByUserIdAndDay(db *gorm.DB) func(userId uuid.UUID, day time.Time) ops.Provider[[]Model] {
	return func(userId uuid.UUID, day time.Time) ops.Provider[[]Model] {
		// Normalize day to date only (strip time component)
		normalized := time.Date(day.Year(), day.Month(), day.Day(), 0, 0, 0, 0, time.UTC)
		return ops.SliceMap(Make)(database.SliceQuery[Entity](db, Entity{UserId: userId, Day: normalized}))(ops.ParallelMap())
	}
}

// GetByUserIdAndStatus returns a provider that fetches all tasks for a user with a specific status
...
// GetByHouseholdId returns a provider that fetches all tasks for a household
...
// GetByUserIdDayRange returns a provider that fetches all tasks for a user within a date range
func GetByUserIdDayRange(db *gorm.DB) func(userId uuid.UUID, startDay, endDay time.Time) ops.Provider[[]Model] {
	return func(userId uuid.UUID, startDay, endDay time.Time) ops.Provider[[]Model] {
		return func() ([]Model, error) {
			var entities []Entity
			err := db.Where("user_id = ? AND day BETWEEN ? AND ?", userId, startDay, endDay).
...
// Count returns a provider that counts total tasks for a user
...
// CountByStatus returns a provider that counts tasks by status for a user
